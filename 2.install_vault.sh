...
Installs HashiCorp Vault in development mode for a simple demo with envconsul tool.
...
function setupVaultExample()
{
	...
	vault secrets enable -version=2 kv 2>&1>/dev/null ;
	if (($? == 0)) ; then printf "SUCCESSFULLY: Enabled Vault KV v2 Engine.\n" ; fi ;

	vault kv put kv/key value="random ${RANDOM}" 2>&1> /dev/null && \
	vault kv put kv/user value="user_rnd${RANDOM}" 2>&1> /dev/null && \
	vault kv put kv/secret value="53cr3t_${RANDOM}" 2>&1> /dev/null ;
	if (($? == 0)) ; then printf "SUCCESSFULLY: Wrote some random data to paths kv/key, kv/user & kv/secret.\n" ; fi ;
}

URL="${URL}${FILE}" ;
donwloadUnpack && if [[ ${SETUP,,} == *"server"* ]]; then sudoSetup && setupVaultExample ; fi ;
